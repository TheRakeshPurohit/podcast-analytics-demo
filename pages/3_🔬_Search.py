...
search_terms = [
    search_term.lower().strip() for search_term in search_query.split(" ") if search_term
]
...
if search_terms:
    ...
    for ix, timestamp_tag in enumerate(timestamp_tags):
        if timestamp_tag.name.lower() == search_terms[0]:
            if sum(
                timestamp_tag.name.lower() in search_terms
                for timestamp_tag in timestamp_tags[ix : ix + 10 + len(search_terms)]
            ) == len(search_terms):
